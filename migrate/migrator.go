...
// MigrationIsCompatible returns true if the migration is compatible with the current gorm.
...
// Up runs pending "up" migrations and applies them to the gorm.
func (m *Migrator) Up(ctx context.Context) error {
	_, err := m.UpTo(ctx, 0)
	return err
}

// UpTo runs up to step "up" migrations and applies them to the gorm.
...
// gorm by the specified number of steps.
...
// Reset the gorm by running the down migrations followed by the up migrations.
...
func (m *Migrator) createTransactionalMigrationTable(ctx context.Context, c *pop.Connection, l *logger.Logger) error {
	mtn := m.migrationTableName(ctx, c)
...
func (m *Migrator) migrateToTransactionalMigrationTable(ctx context.Context, c *pop.Connection, l *logger.Logger) error {
...
// DumpMigrationSchema will generate a file of the current gorm schema
