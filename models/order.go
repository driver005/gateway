...
type Order struct {
	...
	Discounts            []Discount            `json:"discounts"  gorm:"column:discounts;many2many:order_discounts"`
	GiftCards            []GiftCard            `json:"gift_cards"  gorm:"column:gift_cards;many2many:order_gift_cards"`
	ShippingMethods      []ShippingMethod      `json:"shipping_methods"  gorm:"column:shipping_methods;foreignKey:Id"`
	Payments             []Payment             `json:"payments"  gorm:"column:payments;foreignKey:Id"`
	Fulfillments         []Fulfillment         `json:"fulfillments"  gorm:"column:fulfillments;foreignKey:Id"`
	Returns              []Return              `json:"returns"  gorm:"column:returns;foreignKey:Id"`
	Claims               []ClaimOrder          `json:"claims"  gorm:"column:claims;foreignKey:Id"`
	Refunds              []Refund              `json:"refunds"  gorm:"column:refunds;foreignKey:Id"`
	Swaps                []Swap                `json:"swaps"  gorm:"column:swaps;foreignKey:Id"`
	DraftOrderId         uuid.NullUUID         `json:"draft_order_id"  gorm:"column:draft_order_id"`
	DraftOrder           *DraftOrder           `json:"draft_order"  gorm:"column:draft_order;foreignKey:DraftOrderId"`
	Items                []LineItem            `json:"items"  gorm:"column:items;foreignKey:Id"`
	ReturnableItems      []LineItem            `json:"returnable_items"  gorm:"column:returnable_items;foreignKey:Id"`
	Edits                []OrderEdit           `json:"edits"  gorm:"column:edits;foreignKey:Id"`
	GiftCardTransactions []GiftCardTransaction `json:"gift_card_transactions"  gorm:"column:gift_card_transactions;foreignKey:Id"`
