...
// Canonicalize returns constants DriverMySQL, DriverPostgreSQL, DriverCockroachDB, UnknownDriver, depending on `gorm`.
func Canonicalize(gorm string) string {
	switch gorm {
	case "mysql":
		return DriverMySQL
...
func MustCanonicalize(gorm string) string {
	d := Canonicalize(gorm)
	if d == UnknownDriver {
		fmt.Printf("Unknown gorm driver: %s", gorm)
	}
	return d
}
